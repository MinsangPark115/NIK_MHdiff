...
def diffrs_sampler(
    ...
    def sampling_loop(x_next, lst_idx, log_ratio_prev, per_sample_nfe, labels, warmup=False):
        ...
        bool_zero = lst_idx == 0
        print(next(discriminator.parameters()).device, next(vpsde.parameters()).device, x_cur[bool_zero].device, t_steps[lst_idx][bool_zero].device, net.img_resolution.device, time_min.device, time_max.device, labels.device)
        if warmup:
            if bool_zero.sum() != 0:
